      ...
      <div className="max-w-7xl mx-auto space-y-10 pb-12">
        <section className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border/40 pb-8 mt-4">
          <div className="space-y-2">
            ...
            <h1 className="text-2xl font-bold text-foreground tracking-tight">Identity Report</h1>
            <p className="text-[11px] font-bold text-muted-foreground  tracking-wider px-0.5">Verification ID {id?.slice(0,12)}</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 opacity-40"><ShieldCheck className="w-12 h-12 mb-4" /><p className="font-bold  tracking-wider text-sm">Artifact not found</p></div>
        ) : (
          <div className="grid lg:grid-cols-12 gap-10 items-start">
            <div className="lg:col-span-8 space-y-8">
              <section className="bg-background border border-border/60 rounded-2xl p-8 shadow-none">
                 <h3 className="text-sm font-bold  tracking-wider text-muted-foreground/60 mb-8 flex items-center gap-2">
                 ...
                 <div className="grid sm:grid-cols-2 gap-8">
                    ...
                    {documents.map((doc, i) => (
                       <div key={i} className="space-y-6 sm:col-span-2 bg-muted/5 p-6 rounded-xl border border-border/40">
                          <p className="text-[11px] font-bold  tracking-wider text-primary">{doc.type} Evidence</p>
                          ...
                          <div className="pt-4 border-t border-border/40 flex flex-wrap gap-6 text-[11px] font-bold  tracking-wider text-muted-foreground/60">
            ...
            <div className="lg:col-span-4 space-y-6">
               <div className={cn("p-8 rounded-2xl border border-border/60 shadow-none text-center space-y-6", statusTheme.className)}>
                  <div className="inline-flex w-14 h-14 rounded-xl bg-background border border-current/10 items-center justify-center shadow-none">
                     <statusTheme.icon className="w-8 h-8" />
                  </div>
                  <div className="space-y-1">
                     <p className="text-[11px] font-bold  tracking-wider opacity-60">Verification State</p>
                     <h3 className="text-xl font-bold  tracking-tight">{statusTheme.label}</h3>
                  </div>
               ...
               {kyc.rejectionReason && (
                 <div className="p-6 bg-rose-50 border border-rose-100 rounded-2xl text-rose-700">
                    <p className="text-[11px] font-bold  tracking-wider mb-2 flex items-center gap-1.5"><XCircle className="w-3 h-3" /> System Outlier Found</p>
                    <p className="text-sm font-semibold leading-relaxed">{kyc.rejectionReason}</p>
                 </div>
               )}

               <div className="bg-background border border-border/60 rounded-2xl p-6 shadow-none space-y-3">
                  <div className="flex items-center gap-2 text-muted-foreground">
                     <ShieldCheck className="w-4 h-4" />
                     <span className="text-[11px] font-bold  tracking-wider">Digital Assurance</span>
                  </div>
...
function DataPoint({ label, value, colSpan = 1 }: { label: string; value: any; colSpan?: number }) {
  return (
    <div className={cn("space-y-1", colSpan === 2 && "sm:col-span-2", colSpan === 3 && "sm:col-span-3")}>
       <p className="text-[11px] font-bold text-muted-foreground  tracking-wider opacity-60">{label}</p>
...
function ImageArtifact({ label, src, small }: { label: string; src?: string; small?: boolean }) {
  return (
    <div className="space-y-3">
       <p className="text-[11px] font-bold text-muted-foreground  tracking-wider px-1">{label}</p>
       <div className={cn("rounded-xl border border-border/40 bg-muted/10 overflow-hidden flex items-center justify-center shadow-inner", small ? "aspect-video" : "aspect-square")}>
          {src ? (
             ...
             <div className="flex flex-col items-center gap-2 opacity-20"><Upload className="w-6 h-6" /><span className="text-[11px] font-bold  tracking-wider">Awaiting</span></div>
          )}
       </div>
    </div>
  );
