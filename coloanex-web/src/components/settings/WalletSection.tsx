...
export default function WalletSection({
  ...
  useEffect(() => {
    ...
    ethereum.request({ method: "eth_accounts" }).then(updateAccount);
  ...
  const isMetaConnected = wallets.some(
    (w) =>
      w.provider === "METAMASK" &&
      activeAddress &&
  ...
  const primaryWallet =
    ...
    wallets.find((w) => w.provider === "METAMASK");

  return (
    <div className="space-y-6">
      <Card className="rounded-2xl border-border bg-card/30 backdrop-blur-sm overflow-hidden border-t-4 border-t-primary/20 shadow-none">
        ...
        <CardContent className="pt-6 space-y-6">
          <div className={cn(
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className={cn("text-sm font-bold tracking-tight", isMetaConnected ? "text-emerald-600 dark:text-emerald-400" : "text-destructive")}>
          ...
          <div className="space-y-3">
             <Label className="text-[10px] font-bold tracking-wider text-muted-foreground ml-1">Gas execution strategy</Label>
                ...
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      m.active ? "bg-primary/10 border-primary/20" : "bg-background border-border/60 hover:bg-muted/30"
                    )}
                  >
             ...
             {!blockchainAccess.canRunBlockchain && (
               <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 flex items-center gap-2">
                  ...
                  <p className="text-[10px] font-bold text-amber-700 dark:text-amber-400 tracking-tighter">{blockchainAccess.reason}</p>
               </div>
             )}
          </div>
        </CardContent>
      ...
      <Card className="rounded-2xl border-border bg-card/30 backdrop-blur-sm overflow-hidden border-t-4 border-t-primary/20 shadow-none">
        ...
        <CardContent className="pt-6 space-y-6">
          ...
          <div className="space-y-2">
             <Label className="text-[10px] font-bold tracking-wider text-muted-foreground ml-1">Connected identities</Label>
             <div className="grid grid-cols-1 gap-2">
                {wallets.length === 0 ? (
                  <div className="p-8 rounded-2xl border border-dashed border-border/40 bg-muted/5 text-center text-muted-foreground">
                    <p className="text-[10px] font-bold tracking-widest opacity-40">No wallets indexed yet</p>
                  </div>
                ) : (
                  wallets.map((wallet) => (
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                         ...
                         <div className="min-w-0">
                            <div className="flex items-center gap-2">
                               ...
                               {wallet.isPrimary ? (
                                 <span className="text-[8px] px-1.5 py-0.5 rounded bg-primary/10 text-primary font-black tracking-tighter">Primary</span>
                               ) : (
                                 <span className="text-[8px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground font-black tracking-tighter">{wallet.purpose}</span>
                               )}
                            </div>
                            ...
                            {wallet.provider === "METAMASK" && (
                              <p className="text-[9px] font-bold text-emerald-600/70 dark:text-emerald-400/70 tracking-widest mt-0.5">
