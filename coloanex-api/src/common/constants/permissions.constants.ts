export const READ_ROLES = 'Read Roles';
export const CREATE_ROLES = 'Create Roles';
export const UPDATE_ROLES = 'Update Roles';
export const DELETE_ROLES = 'Delete Roles';

export const READ_PERMISSIONS = 'Read Permissions';
export const CREATE_PERMISSIONS = 'Create Permissions';
export const UPDATE_PERMISSIONS = 'Update Permissions';
export const DELETE_PERMISSIONS = 'Delete Permissions';

export const READ_USERS = 'Read Users';
export const CREATE_USERS = 'Create Users';
export const UPDATE_USERS = 'Update Users';
export const DELETE_USERS = 'Delete Users';

export const READ_TENANTS = 'Read Tenants';
export const CREATE_TENANTS = 'Create Tenants';
export const UPDATE_TENANTS = 'Update Tenants';
export const DELETE_TENANTS = 'Delete Tenants';

export const READ_BORROWERS = 'Read Borrowers';
export const CREATE_BORROWERS = 'Create Borrowers';
export const UPDATE_BORROWERS = 'Update Borrowers';
export const DELETE_BORROWERS = 'Delete Borrowers';

export const READ_KYC_DOCUMENTS = 'Read KYC Documents';
export const CREATE_KYC_DOCUMENTS = 'Create KYC Documents';
export const UPDATE_KYC_DOCUMENTS = 'Update KYC Documents';
export const DELETE_KYC_DOCUMENTS = 'Delete KYC Documents';
export const APPROVE_KYC_DOCUMENTS = 'Approve KYC Documents';

export const READ_LOANS = 'Read Loans';
export const CREATE_LOANS = 'Create Loans';
export const UPDATE_LOANS = 'Update Loans';
export const DELETE_LOANS = 'Delete Loans';
export const APPROVE_LOANS = 'Approve Loans';
